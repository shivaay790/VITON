...
# cupy is only needed for the CUDA kernel below. Without it (or on CPU) the
# pure PyTorch implementation further down is used instead, so the model runs
# on any machine.
try:
    import cupy
except ImportError:
...
def correlation_torch(tenFirst, tenSecond, intStride, max_displacement=3):
    """Pure PyTorch equivalent of the CUDA kernel above.

    Output channel (p + 3) * 7 + (o + 3) at output position (y, x) is the
    channel-mean dot product of tenFirst at (y*s, x*s) with tenSecond at
    ((y + p)*s, (x + o)*s), zero outside the image, for p, o in [-3, 3].
    Works on CPU and GPU and is differentiable through ordinary autograd.
    """
    d, s = max_displacement, intStride
    channels = tenFirst.shape[1]
    first = tenFirst[:, :, ::s, ::s]
    out_h, out_w = first.shape[2], first.shape[3]
    pad = d * s
    ...
    for p in range(-d, d + 1):
        for o in range(-d, d + 1):
            y0, x0 = pad + p * s, pad + o * s
            shifted = second[
                :, :, y0 : y0 + (out_h - 1) * s + 1 : s, x0 : x0 + (out_w - 1) * s + 1 : s
            ]
            volumes.append((first * shifted).sum(dim=1) / channels)
    return torch.stack(volumes, dim=1)


def _use_cuda_kernel(tensor):
    return cupy is not None and tensor.is_cuda


def FunctionCorrelation(tenFirst, tenSecond, intStride):
    if not _use_cuda_kernel(tenFirst):
        return correlation_torch(tenFirst, tenSecond, intStride)
...
class ModuleCorrelation(torch.nn.Module):
    ...
    def forward(self, tenFirst, tenSecond, intStride):
        return FunctionCorrelation(tenFirst, tenSecond, intStride)
